...
class local_custom_certification_renderer extends \plugin_renderer_base
{
                                     ...
                                     $capability, $enrolleduser, $isrecertif, $progress, $usercertificationdetails, $ragstatus)
    {
        ...
        if ($enrolleduser) {
            ...
            $output .= html_writer::tag('span', get_string('headerstatus', 'local_custom_certification'), []);

            $output .= html_writer::div('', 'custom-circle', ['class' => 'status-' . $ragstatus]);
            $output .= html_writer::end_div();
            $output .= html_writer::start_div('progress-box', []);
            $output .= html_writer::tag('span', get_string('headerprogress', 'local_custom_certification'), []);
            $output .= html_writer::start_div('custom-progressbar', ['title' => $certificationprogress . '%']);
            $output .= html_writer::div('', 'custom-progress', ['style' => 'width:' . $certificationprogress . '%;', 'class' => 'status-' . $ragstatus]);
        ...
        if($enrolleduser && $certificationprogress == 100 && $ragstatus != \local_custom_certification\completion::RAG_STATUS_GREEN){
            $output .= html_writer::div(get_string('statusnote', 'local_custom_certification'), 'status-note pull-right');
        }


        if ($enrolleduser) {
            $output .= html_writer::start_div('row assignment-info');
            $output .= html_writer::start_div('col-md-4');
            $output .= html_writer::tag('span', get_string('userassignmentdate', 'local_custom_certification', html_writer::span(userdate(array_values($assignmentdata)[0]->assignmenttime, get_string('strftimedate')), 'bold')), ['class' => 'assignmentdate']);
            if ($usercertificationdetails->duedate > 0) {
                $duedate = userdate($usercertificationdetails->duedate, get_string('strftimedate'));
            } else {
                $duedate = get_string('datenotset', 'local_custom_certification');
            }
            ...
            $output .= html_writer::start_div('col-md-8 align-right');
            $output .= html_writer::tag('span', get_string('instructions:completioncriteria', 'local_custom_certification'), ['class' => 'completioncriteriainfo']);

            foreach($assignmentdata as $assignment){
                $output .= html_writer::empty_tag('br');
                if (!empty($assignment->cohortname)) {
                    $output .= html_writer::tag('span', get_string('cohortcompletioncriteria', 'local_custom_certification', $assignment->cohortname).($assignment->duedatetype == certification::DUE_DATE_NOT_SET ? ' ('.get_string('optional', 'local_custom_certification').')' : ''), ['class' => 'bold completioncriteriainfo']);
                } else {
                    $output .= html_writer::tag('span', get_string('individualcompletioncriteria', 'local_custom_certification'), ['class' => 'bold completioncriteriainfo']);
                }
            }
            $output .= html_writer::end_div();
            $output .= html_writer::end_div();
        ...
        if(!empty($certif->summary)){
            $output .= html_writer::tag('h4', get_string('summarylabel', 'local_custom_certification'), ['class' => 'certif-label']);
            $output .= html_writer::tag('p', $certif->summary, ['class' => 'certif-value']);
        }
        $contentfiles = '';
        ...
        if ($enrolleduser) {
            if (!$isrecertif) {
                $output .= html_writer::tag('h4', get_string('mandatorytraining', 'local_custom_certification'), ['class' => 'certif-label']);
                ...
                $output .= html_writer::tag('h4', get_string('mandatorytrainingrevalidation', 'local_custom_certification'), ['class' => 'certif-label recertification-container']);
    ...
    private function overview_courseset($certif, $coursesets, $capability, $enrolleduser, $isrecertif, $progress, $ragstatus)
    {
        ...
        foreach ($coursesets as $courseset) {
            ...
            $output .= html_writer::start_div('coursesetbox overview-coursesetbox', []);

            if ($courseset->completiontype == local_custom_certification\certification::COMPLETION_TYPE_ALL) {
                $completiontypeinfo = get_string('completiontypeallinfo', 'local_custom_certification');
            } elseif ($courseset->completiontype == local_custom_certification\certification::COMPLETION_TYPE_ANY) {
                $completiontypeinfo = get_string('completiontypeanyinfo', 'local_custom_certification');
            } elseif ($courseset->completiontype == \local_custom_certification\certification::COMPLETION_TYPE_SOME) {
                $completiontypeinfo = get_string('completiontypesomeinfo', 'local_custom_certification', $courseset->mincourses);
            }

            $output .= html_writer::tag('h3', $courseset->label . html_writer::tag('span', $completiontypeinfo, []), []);
            ...
            $output .= html_writer::tag('th', get_string('headeractions', 'local_custom_certification'), []);
            ...
            foreach ($courseset->courses as $course) {
                $output .= html_writer::start_tag('tr', []);
                $output .= html_writer::tag('td', $course->fullname, []);
                ...
                if ($capability || $enrolleduser) {
                    ...
                    if ($capability) {
                        ...
                        $urlparams = [];
                        $urlparams['id'] = $course->courseid;
                        $urlparams['certifid'] = $certif->id;
                        $urlparams['certifpath'] = $isrecertif ? \local_custom_certification\certification::CERTIFICATIONPATH_RECERTIFICATION : \local_custom_certification\certification::CERTIFICATIONPATH_BASIC;
                        $url = new moodle_url('/enrol/index.php', $urlparams);
                    }

                    $output .= html_writer::tag('button', get_string('launchcourse', 'local_custom_certification'), ['class' => 'btn-primary', 'onclick' => 'window.location=\''.$url->out(false).'\'']);
                    $output .= html_writer::end_tag('td');
                } else {
                ...
                if ($enrolleduser) {
                    ...
                    if ($courseprogress == 0) {
                        $output .= get_string('coursenotstarted', 'local_custom_certification');
                    } else {
                        $output .= html_writer::start_div('custom-progressbar', ['title' => $courseprogress . '%']);
                        $output .= html_writer::div('', 'custom-progress', ['style' => 'width:' . $courseprogress . '%;', 'class' => 'status-green']);
                        $output .= html_writer::end_div();
                    }
                }
