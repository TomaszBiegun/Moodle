...
class completion
{
     ...
     */
    public static function is_recertification($certificationid, $userid){
        global $DB;
     ...
     * @return int
     */
    public static function get_completion_time($cerification, $userid, $certifpath){
        global $DB;
    ...
    public static function update_completion_status($certification, $userid, $certifpath, $status){
        ...
        if(!$completionrecord){
            ...
            if($status == self::COMPLETION_STATUS_COMPLETED){
                ...
                /**
                 * If certification has recertification path then caculate time expire and window open time
                 */
     ...
     * @param $status Status
     */
    public static function update_courseset_completion_status($coursesetid, $userid, $status){
        global $DB;
        $firecompletionevent = false;
    ...
    public static function open_window($completionrecord){
        ...
        if($DB->insert_record('certif_completions_archive', $insertrecord)){
            ...
            foreach($certification->recertificationcoursesets as $courseset){
                foreach($courseset->courses as $course){
